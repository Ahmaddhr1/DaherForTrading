...
export default function CustomerStatementPage() {
  ...
  const handleShareWhatsApp = () => {
    ...
    const shareUrl = `${window.location.origin}/statement/${id}`;
    const message = `Hello ${statement.fullName || ""}, here is your account statement: ${shareUrl}`;
  ...
  const handleShareThermalPdf = async () => {
    ...
    try {
      ...
      const result = await sharePdfOrDownload(blob, `statement-${statement.fullName?.replace(/\s+/g, "-") || id}.pdf`);
        ...
        <Card className="shadow-sm border-gray-200">
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <Table>
                <TableCaption>{orders.length === 0 ? "No orders on file." : "All orders for this customer"}</TableCaption>
                <TableHeader className="bg-gray-50/50">
                  <TableRow>
                    <TableHead className="font-semibold text-gray-700">Date</TableHead>
                    <TableHead className="text-right font-semibold text-gray-700">Total</TableHead>
                    <TableHead className="text-right font-semibold text-gray-700">Paid</TableHead>
                    <TableHead className="text-right font-semibold text-gray-700">Remaining</TableHead>
                ...
                <TableBody>
                  {orders.map((order, index) => {
                    ...
                    return (
                      <TableRow key={order._id} className={index % 2 === 0 ? "bg-gray-50/30" : ""}>
